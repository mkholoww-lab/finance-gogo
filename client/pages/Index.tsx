...
export default function Index() {
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
        ...
        <section className="rounded-[28px] bg-brand-ink p-6 text-white shadow-[0_20px_50px_rgba(23,35,44,0.16)] sm:p-7">
          <div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-white/45">Касса дня</p><h2 className="mt-2 font-display text-xl font-bold">Что осталось у финансиста</h2></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-brand-teal text-brand-ink"><WalletCards className="h-4 w-4" /></span></div>
  ...
  const renderIncome = () => (
    <>
      <PageHeader title="Доходы" eyebrow="Только деньги компании" description="Здесь собраны только собственные доходы Ma’ruf. Суммы, которые нужно вернуть партнёрам, показаны отдельно и не увеличивают доход." dateValue={dateValue} onDateChange={setDateValue} />
  ...
  const renderShift = () => (
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="flex items-center gap-2 text-xs font-semibold text-brand-muted"><span className="grid h-5 w-5 place-items-center rounded-full bg-brand-teal/15 text-brand-teal-dark"><Check className="h-3 w-3" /></span> Партнёрские деньги не входят в доход компании</p><button type="button" onClick={handleSave} className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-brand-teal px-6 text-sm font-extrabold text-brand-ink shadow-[0_10px_24px_rgba(126,218,192,0.28)] transition hover:-translate-y-0.5 hover:bg-[#a5ead6] active:translate-y-0">{isSaved ? <><Check className="h-4 w-4" /> Смена сохранена</> : <><Banknote className="h-4 w-4" /> Сохранить отчёт</>}</button></div>
      </div>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-brand-mist text-brand-ink lg:flex">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] flex-col bg-brand-ink px-4 py-5 text-white lg:flex">
        <div className="flex items-center gap-3 px-3"><div className="relative grid h-10 w-10 place-items-center overflow-hidden rounded-[14px] bg-brand-teal text-brand-ink"><span className="absolute -right-2 -top-2 h-6 w-6 rounded-full bg-brand-yellow/90" /><span className="relative font-display text-lg font-extrabold">M</span></div><div><p className="font-display text-[17px] font-bold leading-none tracking-tight">Ma’ruf</p><p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/45">финансист</p></div></div>
        ...
        <div className="mt-5 flex items-center gap-3 border-t border-white/10 px-2 pt-5"><div className="grid h-9 w-9 place-items-center rounded-full bg-brand-coral text-xs font-bold text-brand-ink">МК</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">Маруф Холоуов</p><p className="text-xs text-white/40">Финансовый отдел</p></div><Settings2 className="h-4 w-4 text-white/35" /></div>
      </aside>
      <div className="min-w-0 flex-1 lg:pl-[248px]"><header className="flex items-center justify-between border-b border-brand-line/80 bg-white/75 px-4 py-4 backdrop-blur-sm sm:px-6 lg:hidden"><div className="flex items-center gap-2.5"><div className="grid h-9 w-9 place-items-center rounded-xl bg-brand-teal font-display font-extrabold text-brand-ink">M</div><div><p className="font-display text-base font-bold leading-none">Ma’ruf</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[0.18em] text-brand-muted">финансист</p></div></div><div className="flex items-center gap-2"><button type="button" className="grid h-9 w-9 place-items-center rounded-xl border border-brand-line text-brand-muted" aria-label="Уведомления"><Bell className="h-4 w-4" /></button><button type="button" onClick={() => navigate("settings")} className="grid h-9 w-9 place-items-center rounded-xl bg-brand-ink text-white" aria-label="Настройки"><Menu className="h-4 w-4" /></button></div></header>
