...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-brand-mist text-brand-ink lg:flex">
      ...
      <div className="min-w-0 flex-1 lg:pl-[248px]"><header className="flex items-center justify-between border-b border-brand-line/80 bg-white/75 px-4 py-4 backdrop-blur-sm sm:px-6 lg:hidden"><div className="flex items-center gap-2.5"><div className="grid h-9 w-9 place-items-center rounded-xl bg-brand-teal font-display font-extrabold text-brand-ink">M</div><div><p className="font-display text-base font-bold leading-none">Fincance GoGo</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[0.18em] text-brand-muted">учёт и расчёты</p></div></div><div className="flex items-center gap-2"><button type="button" className="grid h-9 w-9 place-items-center rounded-xl border border-brand-line text-brand-muted" aria-label="Уведомления"><Bell className="h-4 w-4" /></button><button type="button" onClick={() => navigate("settings")} className="grid h-9 w-9 place-items-center rounded-xl bg-brand-ink text-white" aria-label="Настройки"><Menu className="h-4 w-4" /></button></div></header>
