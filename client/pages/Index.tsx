  ...
  const renderShift = () => (
    <>
      ...
      <div className="space-y-6">
        <section className="rounded-[28px] border border-brand-line bg-white p-5 shadow-[0_18px_50px_rgba(23,35,44,0.045)] sm:p-7"><SectionLabel eyebrow="01 / кто сдаёт" title="Курьер и дата смены" action={<span className="rounded-full bg-brand-mist px-3 py-1.5 text-[11px] font-bold text-brand-muted">Обязательно</span>} /><div className="relative"><div className="mb-3 flex items-center gap-2 text-xs font-semibold text-brand-muted"><Search className="h-4 w-4 text-brand-teal-dark" /> Поиск курьера по имени · выбран: {selectedCourier?.name ?? "нет"}</div><button type="button" onClick={() => setIsCourierOpen((open) => !open)} className="hidden flex w-full items-center justify-between rounded-2xl border border-brand-line bg-brand-mist/50 px-4 py-3.5 text-left transition-colors hover:border-brand-teal focus:outline-none focus:ring-2 focus:ring-brand-teal/30"><span className="flex items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-xl text-xs font-bold ${selectedCourier?.tone ?? toneOptions[0]}`}>{selectedCourier?.initials ?? "?"}</span><span><span className="block text-[11px] font-semibold text-brand-muted">Выбранный курьер</span><span className="mt-0.5 block text-sm font-bold text-brand-ink">{selectedCourier?.name ?? "Добавьте курьера в настройках"}</span></span></span><ChevronDown className={`h-5 w-5 text-brand-muted transition-transform ${isCourierOpen ? "rotate-180" : ""}`} /></button><div className="mt-3 w-full overflow-hidden rounded-2xl border border-brand-line bg-white p-2 shadow-[0_14px_30px_rgba(23,35,44,0.07)]"><div className="relative mb-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-muted" /><input value={courierSearch} onChange={(event) => setCourierSearch(event.target.value)} placeholder="Начните вводить имя курьера..." className="h-10 w-full rounded-xl bg-brand-mist pl-9 pr-3 text-sm font-medium outline-none ring-brand-teal/30 placeholder:text-brand-muted focus:ring-2" /></div>{filteredCouriers.length > 0 ? filteredCouriers.map((courier) => <button type="button" key={courier.id} onClick={() => { setSelectedCourierId(courier.id); setIsCourierOpen(false); setCourierSearch(""); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-brand-mist"><span className={`grid h-8 w-8 place-items-center rounded-lg text-[10px] font-bold ${courier.tone}`}>{courier.initials}</span><span className="flex-1 text-sm font-semibold">{courier.name}</span>{selectedCourierId === courier.id && <Check className="h-4 w-4 text-brand-teal-dark" />}</button>) : <p className="px-3 py-4 text-center text-sm font-medium text-brand-muted">Курьер не найден</p>}</div></div></section>
